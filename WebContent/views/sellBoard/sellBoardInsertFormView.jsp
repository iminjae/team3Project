...
<html>
  ...
  <body>
    ...
    <main class="mt-3">
	    <form id="insertForm" action="/sellInsert.bo" method="post" enctype="multipart/form-data">
	    <input type="hidden" name="writer" value="<%=loginUser.getUserId()%>">
	      ...
	      <div class="container">
	        ...
	        <div class="mb-3 row">
	          ...
	          <!-- 뒤로 버튼 어디로연결할지 -->
	            <button type="button" class="btn btn-lg btn-outline-secondary" onclick="location.href='<%=request.getContextPath()%>/list.th?currentPage=1';">목록으로</button>
	          </div>
	          <div class="col-6 d-grid p-1">
	            <input class="btn btn-lg btn-outline-primary" type="submit" value="제출하기"></button>
	          </div>
	        </div>
	      </div>
      </form>		
