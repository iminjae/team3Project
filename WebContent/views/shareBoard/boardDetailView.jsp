<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="com.kh.team3.shareBoard.model.vo.* ,com.kh.team3.member.model.vo.*,java.util.ArrayList" %>  
...
<body>
   <%@ include file="../common/menubar.jsp" %>
    <div class="outer">
      ...
      <table align="center">
		<tr>     
            <th width="10%">[<%= b.getCategoryName() %>]</th>
      ...
      <br>
      	<div class="btns" align="right">
        	<button type="button" onclick="location.href='<%=request.getContextPath()%>/list.sbo?currentPage=1';">목록으로</button>
		...
		<script>
			function updateForm(){
				$("#postForm").attr("action", "<%=request.getContextPath()%>/updateForm.sbo");
				$("#postForm").submit();
			}
			
			function deleteBoard(){
				$("#postForm").attr("action", "<%=request.getContextPath()%>/deleteB.sbo");
				$("#postForm").submit();
			}
		</script>
		<br>
...
<!-- 댓글 관련 영역 -->
	...
	<script>
			function deleteReply(){
				$('#form').each(function(){
				    ...
				    $("#form").attr("action", "<%=request.getContextPath()%>/ReplyDelete.sbo");
					$("#form").submit();
	...
	<script>
			function deleteTBoard(){
				$("#postForm").attr("action", "<%=request.getContextPath()%>/sellDelete.sbo");
				$("#postForm").submit();
			}
	</script>
